...
def register_user_course_routes(app, ApiResponse, sanitize_text, sanitize_html, now_iso, logger, parse_json):
    """Register user course data routes."""
    
    @app.route("/api/v1/courses/<course_id>/enroll", methods=["POST"])
    def enroll_in_course(course_id: str):
        ...
        def _enroll():
            try:
                ...
                if existing:
                    return ApiResponse("Already enrolled in this course", status=409, error=True, code="already_enrolled").to_flask()
    ...
    @app.route("/api/v1/courses/<course_id>/user-data", methods=["PATCH"])
    def update_user_course_data(course_id: str):
        ...
        def _update():
            ...
            if not user_course:
                return ApiResponse("Not enrolled in this course", status=404, error=True, code="not_enrolled").to_flask()
    ...
    @app.route("/api/v1/courses/<course_id>/lectures/<lecture_id>/user-data", methods=["PATCH"])
    ...
    @app.route("/api/v1/courses/<course_id>/assignments/<assignment_id>/user-data", methods=["PATCH"])
    def update_user_assignment_data(course_id: str, assignment_id: str):
        ...
        def _update():
            ...
            if not assignment_exists:
                return ApiResponse("Assignment not found", status=404, error=True, code="assignment_not_found").to_flask()
